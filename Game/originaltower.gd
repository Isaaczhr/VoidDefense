    ...
    for tower_data in Globals.data["tower"]["value"]:
        ...
        if new_tower != null:
            ...
            new_tower.global_position = Vector2(
                Globals.data["coords"]["origin"][0] + Globals.data["coords"]["size"] * i,
                Globals.data["coords"]["origin"][1] - Globals.data["coords"]["size"] - 20
            )  # 设置新塔的初始位置
            ...
            # 更新全局变量为0，表示该位置已被占用
            var grid_x = floor((new_tower.global_position.x - Globals.data["coords"]["origin"][0]) / Globals.data["coords"]["size"])
            var grid_y = floor((new_tower.global_position.y - Globals.data["coords"]["origin"][1]) / Globals.data["coords"]["size"])
            Globals.void_area[grid_x][grid_y] = 0
            i += 1
...
func _show_preview_tower():
    if preview_tower == null:
        preview_tower = _create_tower(is_preview=true)  # 调用实际塔的生成逻辑生成预览塔
        if preview_tower != null:
...
func _update_preview_tower_position():
    if preview_tower:
        ...
        if _is_valid_grid_position(x, y):
            # 检查该位置是否可以放置塔
            if Globals.void_area[y][x] != 0:
                # 有效位置，更新预览塔的位置并显示
...
func snap_to_grid():
    ...
    if global_position.distance_to(closest_grid_point) < Globals.data["coords"]["size"]: # 调整对齐距离
...
func _create_new_tower_at_original_position():
    var grid_size = Globals.data["coords"]["size"]
    var grid_origin = Vector2(Globals.data["coords"]["origin"][0], Globals.data["coords"]["origin"][1])
    var x = floor((original_position.x - grid_origin.x) / grid_size)
    var y = floor((original_position.y - grid_origin.y) / grid_size)
	print("Original position: ", original_position)
	print("Grid position: ", x, y)
    # 检查 Globals.void_area[x][y] 是否存在且不为 0
    if _is_valid_grid_position(x, y):
        if Globals.void_area[x][y] != 0:
            var new_tower = _create_tower()
            if new_tower != null:
                ...
                # 复制一个对象放在目标位置，命名为 true_tower_i
                var true_tower = duplicate()
                if true_tower != null:
                    true_tower.name = "true_tower_%d" % tower_count
                    tower_count += 1
                    get_parent().add_child(true_tower)
                    true_tower.global_position = global_position  # 目标位置
                    true_tower.visible = true
                    true_tower.set_script(self.get_script())
                    print("Created true tower: ", true_tower.name, " at position: ", true_tower.global_position)

                # 将原始拖动的父元素放回初始位置
                global_position = original_position
                print("Moved original tower back to original position: ", global_position)

                # 禁止拖动和复制原始塔
                self.disconnect("input_event", Callable(self, "_input"))
        else:
            print("Cannot create tower here. Position already occupied.")
    else:
        print("Cannot create tower here. Invalid grid position.")

# 创建塔的函数，包括预览塔和实际塔
func _create_tower(type: int = 1, is_preview: bool = false) -> Sprite2D:
    var new_tower = duplicate()
    if new_tower == null:
        print("Failed to duplicate tower.")
        return null
    if not is_preview:
...
# 辅助函数：检查网格位置是否有效
func _is_valid_grid_position(x, y):
    return x >= 0 and x < Globals.void_area.size() and y >= 0 and y < Globals.void_area[0].size()